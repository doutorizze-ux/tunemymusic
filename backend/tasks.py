...
@celery.task(bind=True)
def transfer_deezer_to_spotify_task(self, spotify_token, deezer_token, deezer_playlist_id, spotify_playlist_id=None):
    """
    Deezer -> Spotify transfer.
    spotify_token: Spotify user token (must have modify scopes).
    deezer_token: Deezer access token.
    deezer_playlist_id: Source Deezer playlist ID.
    spotify_playlist_id: Target Spotify playlist ID (optional).
    """
    try:
        logger.info(f"Starting Deezer to Spotify transfer: {deezer_playlist_id} -> {spotify_playlist_id or 'Library'}")
        
        deezer_client = DeezerClient(access_token=deezer_token)
        spotify_client = SpotifyClient(api_token=spotify_token)
        ...
        self.update_state(
            ...
            meta={
                ...
                "total": 0,
                "progress": 0,
                "status": "Fetching Deezer playlist...",
            },
        )
        
        tracks = deezer_client.get_playlist_tracks(deezer_playlist_id)
        total_tracks = len(tracks)
        logger.info(f"Found {total_tracks} tracks in Deezer playlist")
        ...
        for i, track in enumerate(tracks):
            try:
                ...
                # Search on Spotify
                spotify_song_data = spotify_client.search_song(track['artist'], track['title'])
                
                success = False
                if spotify_song_data:
                    if spotify_playlist_id:
                        success = spotify_client.add_song_to_playlist(
                            spotify_song_data, spotify_playlist_id
                        )
                    else:
                        success = spotify_client.add_song_to_spotify(spotify_song_data)
                ...
                if success:
                    successful.append({
                        ...
                        "original_track": track['title']
                    })
                else:
                    failed.append({
                        "artist": track['artist'],
                        "track": track['title'],
                        "reason": "Not found on Spotify" if not spotify_song_data else "Failed to add"
                    })
        ...
        logger.error(f"Deezer transfer task failed: {e}", exc_info=True)
        raise


@celery.task(bind=True)
def transfer_amazon_to_spotify_task(self, spotify_token, amazon_token, amazon_playlist_id, spotify_playlist_id=None):
    """
    Amazon Music -> Spotify transfer.
    spotify_token: Spotify user token (must have modify scopes).
    amazon_token: Amazon access token.
    amazon_playlist_id: Source Amazon playlist ID.
    spotify_playlist_id: Target Spotify playlist ID (optional).
    """
    try:
        logger.info(f"Starting Amazon to Spotify transfer: {amazon_playlist_id} -> {spotify_playlist_id or 'Library'}")
        
        amazon_client = AmazonMusicClient(access_token=amazon_token)
        spotify_client = SpotifyClient(api_token=spotify_token)
        
        # 1. Fetch tracks from Amazon
        self.update_state(
            state="PROGRESS",
            meta={
                "current": 0,
                "total": 0,
                "progress": 0,
                "status": "Fetching Amazon playlist...",
            },
        )
        
        tracks = amazon_client.get_playlist_tracks(amazon_playlist_id)
        total_tracks = len(tracks)
        logger.info(f"Found {total_tracks} tracks in Amazon playlist")
        ...
        logger.error(f"Amazon transfer task failed: {e}", exc_info=True)
        raise
